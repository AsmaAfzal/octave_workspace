...
function varargout = lsqcurvefit (varargin)
  ...
  in_args = cat (2, varargin{1}, real (varargin{2}(:)), varargin(3:4));
  ...
  if (nargs >= 6)
    if (! (isempty (varargin{6})))
      settings = optimset (settings, "ubound", varargin{6}(:));          
    endif     
  endif  

  if (nargs >= 7)
    if (isstruct (varargin{7}))
  ...
  if (out_args >= 7)
     varargout{7} = sparse (jacobs (curvefit_out{1}, in_args{1}));
  endif
